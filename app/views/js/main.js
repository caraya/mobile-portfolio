...
let adjectives = ["dark", "color", "whimsical", "shiny", "noise", "apocalyptic", "insulting", "praise", "scientific"];  // types of adjectives for pizza titles
...
let pizzaElementGenerator = function(i) {
...
let resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    for (let i = 0; i < document.querySelectorAll(".randomPizzaContainer").length; i++) {
      let dx = determineDx(document.querySelectorAll(".randomPizzaContainer")[i], size);
      let newwidth = (document.querySelectorAll(".randomPizzaContainer")[i].offsetWidth + dx) + 'px';
...
function updatePositions() {
  ...
  for (let i = 0; i < items.length; i++) {
    let phase = Math.sin((document.body.scrollTop / 1250) + (i % 5));
